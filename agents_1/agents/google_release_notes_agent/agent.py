...
from . import tools
from google_release_notes_agent.tools import initial_bq_nl2sql, run_bigquery_validation,get_database_settings,github_mcp_tools
...
def setup_bigquery_config(callback_context: CallbackContext):
...
root_agent = Agent(
    ...
    instruction="""
 You are an AI assistant serving as a 
   A. SQL expert for BigQuery.
      ...
      4. Generate the final result in JSON format with four keys: "explain", "sql", "sql_results", "nl_results".
          ...
          "sql_results": "raw sql execution query_result from run_bigquery_validation if it's available, Show the data in a proper tabular format",
